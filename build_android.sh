...
# ndk-build puts executables/static-deps under obj; copy any expected artifact
# that landed in obj but not libs so install_android.sh finds everything.
ensure_artifact() {
    local name=$1
    if [ -f "$LIBS_DIR/$name" ]; then
        log_ok "$name ($(du -h "$LIBS_DIR/$name" | cut -f1))"
    elif [ -f "$OBJ_DIR/$name" ]; then
        mkdir -p "$LIBS_DIR"
        cp "$OBJ_DIR/$name" "$LIBS_DIR/"
        [ -x "$OBJ_DIR/$name" ] && chmod +x "$LIBS_DIR/$name"
        log_ok "$name ($(du -h "$LIBS_DIR/$name" | cut -f1)) (copied from obj)"
    else
