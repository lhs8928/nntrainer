# quick.ai Reference Tests — Android Verification Report
...
- `Applications/quick_ai/meson.build:250` lists `'android'` in `model_unittest_platforms`, but that code is **dead** — it is never reached from a meson Android build.
- `jni/Android.mk` only builds `quick_ai_core`, `nntr_quantize`, `test_api`, and `nntr_safetensors_info` — no gtest, no test sources.
...
Switch to the FP32-accumulator variant — the same fix already applied to the quick.ai layers:
