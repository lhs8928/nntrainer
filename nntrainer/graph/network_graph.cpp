    ...
    for (unsigned int i = 0; i < out_specs.size(); ++i) {
      ...
      if (shared_var) {
        ...
        if (lnode->getType() == IdentityLayer::type) {
          s.variable_spec.reference_name = !inputs.empty() ? inputs[i]->getName() : "";
          s.variable_spec.dim.setFormat(!inputs.empty() ? inputs[i]->getDim().getFormat() : nntrainer::Tformat::NCHW);
        } else if (lnode->getInPlaceDirection() == InPlaceDirection::RIGHT) {
          s.variable_spec.reference_name = inputs.size() > 1 ? inputs[1]->getName() : "";
          s.variable_spec.dim.setFormat(inputs.size() > 1 ? inputs[1]->getDim().getFormat() : nntrainer::Tformat::NCHW);
          ...
          s.variable_spec.reference_name = !inputs.empty() ? inputs[0]->getName() : "";
          s.variable_spec.dim.setFormat(!inputs.empty() ? inputs[0]->getDim().getFormat() : nntrainer::Tformat::NCHW);
        }
      }
      if (shared_grad && s.gradient_spec) {
        s.gradient_spec->request_type =
          TensorSpecV2::RequestType::READ_ONLY_VIEW;
        if (lnode->getType() == IdentityLayer::type) {
          s.gradient_spec->reference_name = !inputs.empty() ? inputs[i]->getGradientName() : "";
          s.gradient_spec->dim.setFormat(!inputs.empty() ? inputs[i]->getDim().getFormat() : nntrainer::Tformat::NCHW);
        } else if (lnode->getInPlaceDirection() == InPlaceDirection::RIGHT) {
          s.gradient_spec->reference_name = inputs.size() > 1 ? inputs[1]->getGradientName() : "";
          s.gradient_spec->dim.setFormat(inputs.size() > 1 ? inputs[1]->getDim().getFormat() : nntrainer::Tformat::NCHW);
        } else if (lnode->getType() == WeightLayer::type) {
          WeightSpec w_spec = init_context.getWeightsSpec()[i];
          ...
          s.gradient_spec->reference_name = !inputs.empty() ? inputs[0]->getGradientName() : "";
          s.gradient_spec->dim.setFormat(!inputs.empty() ? inputs[0]->getDim().getFormat() : nntrainer::Tformat::NCHW);
        }
      }
    }
  }
