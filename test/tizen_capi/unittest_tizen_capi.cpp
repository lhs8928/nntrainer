...
TEST(nntrainer_capi_nnmodel, get_input_output_dimension_01_p) {
  ...
  ml_tensors_info_create(&input_info);
  ml_tensors_info_create(&output_info);
  ...
  status = ml_train_model_get_input_dimensions(handle, &input_info);
  EXPECT_EQ(status, ML_ERROR_NONE);
  ...
  status = ml_train_model_get_output_dimensions(handle, &output_info);
  EXPECT_EQ(status, ML_ERROR_NONE);
...
TEST(nntrainer_capi_nnmodel, get_input_output_dimension_02_n) {
  ml_train_model_h handle = NULL;
  ml_tensors_info_h input_info, output_info;
  ...
  ScopedIni s("test_get_input_dimension_02_n",
              {model_base, optimizer, dataset, inputlayer, outputlayer});
  status = ml_train_model_construct_with_conf(s.getIniName().c_str(), &handle);
  EXPECT_EQ(status, ML_ERROR_NONE);

  EXPECT_THROW(ml_train_model_get_input_dimensions(handle, &input_info),
               std::invalid_argument);
  EXPECT_THROW(ml_train_model_get_output_dimensions(handle, &output_info),
               std::invalid_argument);
