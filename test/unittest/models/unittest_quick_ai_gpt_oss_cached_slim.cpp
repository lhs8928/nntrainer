 ...
 *
 * @file   unittest_quick_ai_gpt_oss_cached_slim.cpp
 * @date   15 June 2026
...
using TinyGptOssCachedSlimCausalLM =
  quick_ai_test::CausalLMTestAdapter<quick_ai::GptOssCachedSlimCausalLM>;

/**
 * @brief Make the tiny GptOssCachedSlim model config
 */
quick_ai::json makeTinyGptOssCachedSlimConfig() {
  return {
...
TEST(GptOssCachedSlimTinyModelTest, GreedyGenerationSelectsArgmaxLogit) {
  auto tokenizer_path =
    quick_ai_test::makeTinyCausalLMFiles("GptOssCachedSlimTinyModelTest",
...
TEST(GptOssCachedSlimTinyModelTest,
  ...
  auto gen_cfg = quick_ai_test::makeTinyGenerationConfig();
  auto nntr_cfg = quick_ai_test::makeTinyNntrainerConfig(
    tokenizer_path, quick_ai_test::makeTinyQ40Fp16DataType());
