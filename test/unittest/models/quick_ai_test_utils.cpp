...
TinyCausalLMFiles makeTinyCausalLMFiles(const std::string &suite_name,
  ...
  std::string name = "nntrainer_quick_ai_tiny";
  name += "_";
  ...
  files.weight_path = dir / "quick_ai_tiny.bin";
  return files;
}
...
std::filesystem::path findFixtureDir(const std::string &fixture_name) {
  if (const char *env = std::getenv("NNTRAINER_QUICK_AI_FIXTURE_DIR")) {
  ...
  return src.parent_path() / "quick_ai_reference" / fixture_name;
}
...
FixtureConfigs loadFixtureConfigs(const std::filesystem::path &dir) {
  FixtureConfigs fc;
  fc.model_cfg = quick_ai::LoadJsonFile((dir / "config.json").string());
  fc.gen_cfg =
    quick_ai::LoadJsonFile((dir / "generation_config.json").string());
  fc.nntr_cfg = quick_ai::LoadJsonFile((dir / "nntr_config.json").string());
...
void runQ40DifferentialChecks(const DifferentialModel &model) {
  ...
  // --- Load Q4_0 model ---
  auto fc = loadFixtureConfigs(fixture_dir);
  auto q4_nntr_cfg = quick_ai::LoadJsonFile(q4_nntr_cfg_path.string());
