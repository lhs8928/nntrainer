...
# Configuration
INSTALL_DIR="/data/local/tmp/nntrainer/quick_ai"
...
OPTIONAL_FILES=(
    "$SCRIPT_DIR/jni/libs/arm64-v8a/libquick_ai_api.so"
...
adb push "$SCRIPT_DIR/jni/libs/arm64-v8a/nntrainer_quick_ai" "$INSTALL_DIR/" 2>&1 | tail -1
adb shell "chmod 755 $INSTALL_DIR/nntrainer_quick_ai"
log_success "nntrainer_quick_ai pushed"
...
log_info "  [1/5] libquick_ai_core.so (CausalLM Core library)..."
adb push "$SCRIPT_DIR/jni/libs/arm64-v8a/libquick_ai_core.so" "$INSTALL_DIR/" 2>&1 | tail -1
...
log_info "  [5/5] libquick_ai_api.so (CausalLM API library)..."
if [ -f "$SCRIPT_DIR/jni/libs/arm64-v8a/libquick_ai_api.so" ]; then
    adb push "$SCRIPT_DIR/jni/libs/arm64-v8a/libquick_ai_api.so" "$INSTALL_DIR/" 2>&1 | tail -1
else
    log_warning "libquick_ai_api.so not found (Optional, skipping)"
fi
...
adb shell "cat > $INSTALL_DIR/run_quick_ai.sh << 'EOF'
#!/system/bin/sh
...
EOF
"
adb shell "chmod 755 $INSTALL_DIR/run_quick_ai.sh"
...
log_info "  - nntrainer_quick_ai (executable)"
if [ -f "$SCRIPT_DIR/jni/libs/arm64-v8a/test_api" ]; then
    log_info "  - test_api (executable)"
fi
log_info "  - libquick_ai_core.so (CausalLM Core library)"
if [ -f "$SCRIPT_DIR/jni/libs/arm64-v8a/libquick_ai_api.so" ]; then
    log_info "  - libquick_ai_api.so (CausalLM API library)"
fi
...
log_info "   adb shell $INSTALL_DIR/run_quick_ai.sh $MODEL_DIR/qwen3-4b"
log_info ""
...
log_info "   ./run_quick_ai.sh $MODEL_DIR/qwen3-4b"
if [ -f "$SCRIPT_DIR/jni/libs/arm64-v8a/test_api" ]; then
