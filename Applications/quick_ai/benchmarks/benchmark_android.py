...
Example:
  python3 benchmark_android.py -m /data/local/tmp/nntrainer/quick_ai/models/qwen3-0.6b -t 1,2,4,8 -n 128,256
"""
...
def run_single_trial(model_path, omp_threads=None):
    ...
    if omp_threads:
        cmd = [
            "adb", "shell",
            f"cd /data/local/tmp/nntrainer/quick_ai && OMP_NUM_THREADS={omp_threads} ./run_quick_ai.sh '{model_path}'"
        ]
    else:
        cmd = [
            "adb", "shell",
            f"cd /data/local/tmp/nntrainer/quick_ai && ./run_quick_ai.sh '{model_path}'"
        ]
...
def validate_model_path(model_path):
    ...
    # Define allowed prefix (must be within nntrainer quick_ai directory)
...
def main():
    ...
    parser.add_argument("-m", "--model", type=str, required=True,
                        help="Model directory path (on device, e.g., /data/local/tmp/nntrainer/quick_ai/models/qwen3-0.6b-q40)")
