...
- nntrainer C++ binary built and deployed to device at `/data/local/tmp/nntrainer/quick_ai/`
...
python3 benchmark_android.py \
  -m /data/local/tmp/nntrainer/quick_ai/qwen3-0.6b \
  -p 512 \
  -n 128 \
  -t 4
```
...
The script requires all model files to be on the device. Ensure your model is deployed to `/data/local/tmp/nntrainer/quick_ai/`.
...
```
/data/local/tmp/nntrainer/quick_ai/
├── models/qwen3-0.6b/
│   ├── nntr_config.json
│   ├── config.json
