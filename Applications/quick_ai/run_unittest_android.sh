...
INSTALL_DIR="/data/local/tmp/nntr_quick_ai_test"
...
if [[ "$USE_CACHE" -eq 1 && -f "$OBJ_DIR/unittest_quick_ai_models" ]]; then
    log_info "Reusing cached ndk-build artifacts"
else
    log_info "Building quick_ai_core, nntr_quantize, unittest_quick_ai_models..."
    ndk-build \
        -C "$JNI_DIR" \
...
for f in unittest_quick_ai_models nntr_quantize libquick_ai_core.so; do
...
adb shell "mkdir -p $INSTALL_DIR/quick_ai_reference $INSTALL_DIR/tmp"

log_info "Pushing binaries and libraries..."
adb push "$OBJ_DIR/unittest_quick_ai_models"  "$INSTALL_DIR/" 2>&1 | tail -1
adb push "$OBJ_DIR/nntr_quantize"             "$INSTALL_DIR/" 2>&1 | tail -1
adb push "$OBJ_DIR/libquick_ai_core.so"       "$INSTALL_DIR/" 2>&1 | tail -1
...
adb shell "chmod 755 $INSTALL_DIR/unittest_quick_ai_models $INSTALL_DIR/nntr_quantize"
