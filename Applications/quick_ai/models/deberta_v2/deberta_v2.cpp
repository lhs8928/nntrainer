  ...
  try {
    if (nntr_cfg.contains("tokenizer_file")) {
      ...
      if (std::filesystem::exists(encoder_config_path)) {
        ...
        loaded_encoder_cfg =
          quick_ai::LoadJsonFile(encoder_config_path.string());
...
void DebertaV2::registerCustomLayers() {
  ...
  try {
    app_context->registerFactory(
      nntrainer::createLayer<quick_ai::DebertaAttentionLayer>);
    app_context->registerFactory(
      nntrainer::createLayer<quick_ai::SharedFullyConnectedLayer>);
