...
The CausalLM API is built as a separate shared library `libquick_ai_api.so`, which depends on the core logic in `libquick_ai_core.so`.

### Build Artifacts (Android)

- **`libquick_ai_core.so`**: Contains the core LLM implementation (Model, Layers, etc.).
- **`libquick_ai_api.so`**: Contains the C API implementation (`causal_lm_api.cpp`) and configuration helpers (`model_config.cpp`).

### Linking

When integrating this API into your application (e.g., via JNI), you must link against both libraries:
1.  `libquick_ai_api.so`
2.  `libquick_ai_core.so`
