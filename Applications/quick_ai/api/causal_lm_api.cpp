...
static quick_ai::CausalLM *g_active_model = nullptr;
...
static std::unique_ptr<quick_ai::ChatTemplate> g_chat_template;
...
public:
  explicit ActiveRunGuard(quick_ai::CausalLM *model) : model_(model) {
    if (model_ != nullptr) {
      ...
      if (g_model_registry.find(lookup_key) != g_model_registry.end()) {
        ...
        if (check_file_exists(resolved_path)) {
          ...
          if (has_config && has_nntr) {
            ...
            try {
              json nntr =
                quick_ai::LoadJsonFile(resolved_path + "/nntr_config.json");
...
ErrorCode loadModel(BackendType compute, ModelType modeltype,
  ...
  try {
    ...
    if (g_model_registry.find(lookup_name) != g_model_registry.end()) {
      ...
      cfg = quick_ai::LoadJsonFile(model_dir_path + "/config.json");
      generation_cfg =
        quick_ai::LoadJsonFile(model_dir_path + "/generation_config.json");
      nntr_cfg = quick_ai::LoadJsonFile(model_dir_path + "/nntr_config.json");
    ...
    if (quick_ai::ChatTemplate::Exists(model_dir_path)) {
      try {
        g_chat_template = std::make_unique<quick_ai::ChatTemplate>(
          quick_ai::ChatTemplate::Load(model_dir_path));
    ...
    g_initialized.store(false, std::memory_order_release);
    g_model = quick_ai::Factory::Instance().create(architecture, cfg,
...
ErrorCode runModel(const char *inputTextPrompt, const char **outputText) {
  ...
  try {
    ...
    auto *causal_lm_model = dynamic_cast<quick_ai::CausalLM *>(g_model.get());
