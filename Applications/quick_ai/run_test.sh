...
# Step 3: Check libquick_ai.so
log_step "3" "6" "Check libquick_ai.so"
LIB_CAUSALLM="$SCRIPT_DIR/jni/libs/arm64-v8a/libquick_ai.so"
log_info "Checking: $LIB_CAUSALLM"

if [ ! -f "$LIB_CAUSALLM" ]; then
    log_error "libquick_ai.so not found"
    log_error "Please run: ./build_quick_ai_lib.sh"
    exit 1
fi

LIB_SIZE=$(du -h "$LIB_CAUSALLM" | cut -f1)
log_success "libquick_ai.so found (size: $LIB_SIZE)"
...
# Push libraries
log_info "  [2/6] libquick_ai.so..."
adb push libquick_ai.so "$INSTALL_DIR/" || true
