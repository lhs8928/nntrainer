...
# This script builds libquick_ai_api.so only
set -e
...
if [ ! -f "$SCRIPT_DIR/jni/libs/arm64-v8a/libquick_ai_core.so" ]; then
    log_error "libquick_ai_core.so not found."
...
log_info "Building with ndk-build (builds libquick_ai_api.so)..."
if ndk-build NDK_PROJECT_PATH=. NDK_LIBS_OUT=./libs NDK_OUT=./obj APP_BUILD_SCRIPT=./Android.mk NDK_APPLICATION_MK=./Application.mk quick_ai_api -j $(nproc); then
...
check_artifact "libquick_ai_api.so" || exit 1
...
echo ""
echo "Libraries:"
echo "  - libquick_ai_api.so (CausalLM API library)"
echo ""
