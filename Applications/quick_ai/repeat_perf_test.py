...
def get_process_count():
    try:
        cmd = ["adb", "shell", "ps -ef | grep nntrainer_quick_ai | grep -v grep | wc -l"]
        result = subprocess.run(cmd, capture_output=True, text=True)
        if result.returncode == 0:
...
def parse_output(output):
    """Parse prefill/generation tokens, ms, TPS from quick_ai output."""
...
def run_quick_ai(model_path, omp_threads=None, taskset_mask=None, input_prompt=None):
    """Run quick_ai binary on device and return stdout."""
    ...
    cmd = [
        "adb", "shell",
        f"cd /data/local/tmp/nntrainer/quick_ai && "
        f"{export_cmd}{taskset_cmd}./run_quick_ai.sh {model_path}{prompt_arg}"
    ]
...
def run_benchmark(run_id, model_path, omp_threads=None, taskset_mask=None):
    ...
    out1, err1, rc1 = run_quick_ai(model_path, omp_threads, taskset_mask)
    ...
    out2, err2, rc2 = run_quick_ai(model_path, omp_threads, taskset_mask,
                                   input_prompt="test")
