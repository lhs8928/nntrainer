...
NNTR_PREBUILT_DIR="$SCRIPT_DIR/builddir/android_build_result/lib/arm64-v8a"
...
resolve() {
    local name=$1
    # libnntrainer.so / libccapi-nntrainer.so are PREBUILT deps; ndk-build may
    # leave a stale copy in libs/ on an incremental build. The authoritative,
    # freshly cross-compiled copy is android_build_result — always prefer it so
    # we never push a stale runtime lib against a fresh libcausallm_core.so.
    case "$name" in
        libnntrainer.so|libccapi-nntrainer.so)
            if [ -f "$NNTR_PREBUILT_DIR/$name" ]; then
                cp -p "$NNTR_PREBUILT_DIR/$name" "$LIBS_DIR/$name"
                return 0
            fi ;;
    esac
